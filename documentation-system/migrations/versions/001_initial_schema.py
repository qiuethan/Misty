...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("url_patterns", ARRAY(sa.Text), nullable=False, server_default=sa.text("ARRAY[]::text[]")),
        ...
        sa.Column("content_fetch_enabled", sa.Boolean, nullable=False, server_default=sa.text("false")),
    ...
    op.create_table(
        ...
        sa.Column("source_id", sa.Text, sa.ForeignKey("sources.id"), nullable=False, server_default=sa.text("'web'")),
    ...
    op.create_table(
        ...
        sa.Column("scopes", ARRAY(sa.Text), nullable=False, server_default=sa.text("ARRAY[]::text[]")),
        sa.Column("active", sa.Boolean, nullable=False, server_default=sa.text("true")),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.Column("created_by", sa.Text, nullable=False),
...
def downgrade() -> None:
    ...
    for ix in ("ix_docs_source", "ix_docs_owning_person", "ix_docs_owning_team", "ix_docs_url_normalized"):
