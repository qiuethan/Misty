...
def verify_production_secrets(settings: Settings | None = None) -> None:
    ...
    is the committed default it is publicly known. `directory_api_key` and
    `connectors_api_key` are outbound credentials this service presents to the
    directory and connectors services respectively; a default there is a
    broken prod config. Called from create_app(), so a misconfigured deploy
    dies at startup, not on first request.
    """
    ...
    if settings.connectors_api_key == DEFAULT_DEV_API_KEY:
        insecure.append("CONNECTORS_API_KEY")
    if insecure:
        raise RuntimeError(
