...
def test_migration_004_backfills_duplicates_then_indexes():
    ...
    try:
        ...
        with engine.begin() as conn:
            ...
            ), {"u": "https://mig.com", "n": "https://mig.com", "c": older}).scalar_one()
            conn.execute(text(
                "INSERT INTO docs (url, url_normalized, source_id, active, created_at, "
                "updated_at, created_by, updated_by) VALUES "
        ...
        with engine.connect() as conn:
            active_ids = conn.execute(text(
                "SELECT id FROM docs WHERE url_normalized = :n AND active"
            ), {"n": "https://mig.com"}).scalars().all()
            assert active_ids == [keep]  # exactly one, and it's the earliest

            index_exists = conn.execute(text(
                "SELECT 1 FROM pg_indexes WHERE indexname = 'uq_docs_url_normalized_active'"
            )).scalar_one_or_none()
