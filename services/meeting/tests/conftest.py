...
def _no_dotenv(monkeypatch):
    ...
    LLM_API_KEY), producing failures unrelated to the change under test.
    Neutralizing env_file here -- before any cached Settings are built --
    ...
    LLM_API_KEY in their shell). Neither leaks a secret (these credentials no
    longer stringify), so this is a correctness/isolation gap rather than a
    security one, but "hermetic" overstates what's actually guaranteed.
    """
